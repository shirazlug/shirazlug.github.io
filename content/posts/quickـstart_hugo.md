...
-شاخه ی data : این دایرکتوری فایل های پیکربندی ای را نگهداری می کند که در زمان اجرای وب سایت ، هیوگو می تواند از آنها استفاده کند. شما می توانید این فایل ها را با فرمت های YAML, JSON , TOML بنویسید.

-شاخه ی layouts : محتوایه این دایرکتوری برای تعیین چگونگی تبدیل محتوا برای نمایش در سایت استفاده میشوند. ( بعدا بیشتر خواهید آموخت )

-شاخه ی statics : این دایرکتوری شامل تمام محتوا هایه ثابت ( static ) نظیر عکس ها ، فایل های css, javascript , … می باشد.
...
۱ – همان طور که در خروجی `hugo server` مشاهده می کنید ، هیوگو پست های پیشنویس (draft) را ایجاد نمی کند.
...
برای اجرای سایت به همراه نمایش فایل های پیشنویس دستور زیر را وارد می کنیم.
...
تا اینجا ما از یک عکس به عنوان پیشفرض برای سایتمان استفاده کرده ایم، اما شاید بخواهیم برای هر کتابی از یک عکس استفاده کنیم. برین منظور تغییر زیر را در فایل firstpost.md ایجاد می کنیم.
...
در عرض کمتر از ده دقیقه شما می توانید تغییرات را در سایت خود واقع در آدرس زیر مشاهده کنید.

<p style="text-align: left; direction: ltr"><code>https://&lt;github-username&gt;.github.io/bookshelf/</code></p>
